...
boot('cli')
    .then(({ServiceService, service, onHalt}) => {
        ...
        onHalt.server.push(async function waitForCommand(signal) {
            await commandRun.halt(signal)
            // todo: a "onHalted" in `commandRun` would be a better style, which must either execute directly or once halted,
            //      or is it a problem that "handleHalt" for CLI should only "mark-as-halting and handle force-shutdown" - but not the server things itself,
            //      which will be halted anyhow at "command finished"
            do {
